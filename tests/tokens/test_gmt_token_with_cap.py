...
class TestContract(AbstractTestContracts):
    """
    run test with python -m unittest tests.tokens.test_gmt_token_with_cap
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.maxCapEndBlock = 4097906 + 1000
        self.gmt_token= self.create_contract('Tokens/GMTokenWithCapFlattened.sol',
    ...
    def test_create_tokens_more_than_total_supply(self):
        ...
        self.c.head_state.block_number = self.startBlock + 1001
    ...
    def test_create_tokens_more_than_cap(self):
        self.gmt_token.startSale()
        # Move forward a few blocks to be within funding time frame, and within max cap timeframe
        self.c.head_state.block_number = self.startBlock + 100

        buyer_1 = 3
        value_1 = 500 * 10**18 # 500 Ether (this would be way above individual max cap)
        buyer_1_tokens = value_1 * self.exchangeRate
        # Register user for participation
        self.gmt_token.changeRegistrationStatus(accounts[buyer_1], True)
    ...
    def test_create_tokens_during_cap_period(self):
        self.gmt_token.startSale()
        # Move forward a few blocks to be within funding time frame, and within max cap timeframe
        self.c.head_state.block_number = self.startBlock + 100

        buyer_1 = 3
        value_1 = 1 * 10**18 # 5 Ether (this would NOT be above individual max cap)
        buyer_1_tokens = value_1 * self.exchangeRate
        # Register user for participation
        self.gmt_token.changeRegistrationStatus(accounts[buyer_1], True)
    ...
    def test_create_tokens_after_cap_period(self):
        self.gmt_token.startSale()
        # Move forward a few blocks to be within funding time frame
        self.c.head_state.block_number = self.startBlock + 1001

        buyer_1 = 3
        value_1 = 10 * 10**18 # 1 Ether (this would be above the individual max cap)
        buyer_1_tokens = value_1 * self.exchangeRate
        # Register user for participation
        self.gmt_token.changeRegistrationStatus(accounts[buyer_1], True)
