...
contract GMTokenWithCap is StandardToken {
    ...
    */
    ...
    uint256 public maxCapEndBlock;  // Block number when min cap period ends
    ...
    uint256 public constant userCap = 10000 * 10**decimals;  // Max GMT tokens a registered user is allowed to purchase
    ...
    *  Constructor
    */
    function GMTokenWithCap(
        address _ethFundAddress,
        address _gmtFundAddress,
        uint256 _startBlock,
        uint256 _endBlock,
        uint256 _maxCapEndBlock) 
        public 
    {
        ...
        endBlock = _endBlock;
        maxCapEndBlock = _maxCapEndBlock;
    ...
    function claimTokens() respectTimeFrame registeredUser atStage(Stages.InProgress) payable external {
        ...
        if (block.number < maxCapEndBlock) {
          // Check user max cap has not been reached
          uint256 currentTokenBalance = balances[msg.sender]; 
          require(currentTokenBalance + tokens <= userCap);
        }
